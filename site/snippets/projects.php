...
<section class="site-content">
  ...
  <?php foreach(page('projects')->children()->visible() as $project): ?>
      <article class="project cf">
        <a href="<?php echo $project->url() ?>.html" title="<?php echo html($project->title()) ?>">

          <?php if($thumb = $project->images()->sortBy('sort', 'asc')->find((string)$project->thumbimage())): ?>
            <div class="thumb">
              <img data-original="<?php echo thumb($thumb, array('width' => 800, 'height' => 800, 'crop' => true))->url() ?>"  alt="<?php echo $thumb->alt() ?>">
            </div>
          <?php endif ?>
